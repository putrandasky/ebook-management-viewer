...
class EbookReaderController extends Controller
{
    ...
    {
        $newpathToFile = Storage::disk('local')->getDriver()->getAdapter()->applyPathPrefix("storage/shared/{$imageName}");
        return response()->file($newpathToFile);
    }
    public function showPdf($pdfname)
    {
        $newpathToFile = Storage::disk('local')->getDriver()->getAdapter()->applyPathPrefix("storage/pdf/{$pdfname}");
        return response()->file($newpathToFile);
    }
